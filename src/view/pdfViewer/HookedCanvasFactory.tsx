...
export default class HookedCanvasFactory {
  ...
  glyphFillText(ctx, text, x, y, glyphs, glyphIdx) {
      const textCommand = new ShapeCommand();
  ...
  strokeText(ctx, text, x, y, maxWidth = undefined) {
      console.log("strokeText not supported");
  }
  ...
  rect(ctx, x, y, width, height) {
      //console.log("rect not supported");
  }
  ...
  unhookCtx(ctx) {
      for (const key in ctx.origFunctions) {
  ...
  uninstallHook() {
      for (const ctx of this.hookedCtxs) {
          this.unhookCtx(ctx);
      }
  }
}